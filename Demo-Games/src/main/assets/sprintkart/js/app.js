...
function initMapScreen() {
	...
	//+ouley
	var buttons = hn.gui.getLayer("map").buttons;
	for (var ii = 0; ii < 9; ii++) {
		buttons[ii].hidden = ii > levelNum;
	}
	hn.gui.pushLayer("map");
}
function initGame() {
    gameState = "game",
    setLogoBut("game"),
...
function initLevelComplete() {
    ...
    //console.log(saveDataHandler.aLevelStore[3 * levelNum + 1], racePos),
    ...
    //console.log(saveDataHandler.aLevelStore),
    saveDataHandler.saveData(),
    previousTime = (new Date).getTime(),
    updateLevelComplete();

	//ouley
	hn.gui.pushLayer("complete");
}
...
//+ouley
function updateGamePause() {
	if (manualPause || rotatePause) {
		panel.render(ctx)
		renderMuteBut();
		requestAnimFrame(updateGamePause)
	}
}
...
function loadAssets() {
    ...
    {
        id: "userCar",
        file: "images/userCar_122x127.png"
    },
    {
        id: "enemyCar",
        file: "images/enemyCar_122x123.png"
...
function toggleManualPause() {
    if (manualPause) manualPause = !1,
        ...
        var a = {
            oImgData: assetLib.getData("uiButs"),
            //aPos: [140, canvas.height / 2],   //-ouley
			aPos: [canvas.width / 2, canvas.height / 2 + 120],
            id: "quit"
        },
		b = {
			oImgData: assetLib.getData("uiButs"),
        ...
        panel = new Elements.Panel(assetLib.getData("panels"), assetLib.getData("uiElements"), assetLib.getData("position"), assetLib.getData("numbers"), "pause", d, canvas.width, canvas.height),
        panel.render(ctx),
        userInput.addHitArea("pause", butEventHandler, null, "rect", {
            aRect: [587, 0, 635, 54]
        }, !0);
...
function (a) {
    var b = function () {
        ...
        a.prototype.getSpriteSize = function (a) {
            for (var b = new Array, c = "", d = "", e = 0, f = a.lastIndexOf("."), g = !0; g;) f--,
    ...
    var b = function () {
        function a(a, b) {
            ...
            a.addEventListener("touchmove", function (a) {
                for (var b = 0; b < a.changedTouches.length; b++) c.move(a, a.changedTouches[b].pageX, a.changedTouches[b].pageY, a.changedTouches[b].identifier, !0)
            }, !1),
        ...
        a.prototype.render = function (a, b) {
            ...
            case "upgrade":
                ...
                for (var f = this.oScoreData.winnings, g = 0; g < f.toString().length; g++) {
                    ...
                    var d = c * this.oNumbersImgData.oData.spriteWidth % this.oNumbersImgData.img.width,
                        e = Math.floor(c / (this.oNumbersImgData.img.width / this.oNumbersImgData.oData.spriteWidth)) * this.oNumbersImgData.oData.spriteHeight;
                    a.drawImage(this.oNumbersImgData.img, d, e, this.oNumbersImgData.oData.spriteWidth, this.oNumbersImgData.oData.spriteHeight, 435 + g * this.numberSpace, 7 + this.posY, this.oNumbersImgData.oData.spriteWidth, this.oNumbersImgData.oData.spriteHeight)
                }
                for (var f = this.oScoreData.totalScore, g = 0; g < f.toString().length; g++) {
                    c = parseFloat(f.toString().charAt(g));
                    var d = c * this.oNumbersImgData.oData.spriteWidth % this.oNumbersImgData.img.width,
...
function (a) {
    var b = function () {
        function b(a, b, c, d, e, f, g, h, i) {
            this.segNum =400;	//ouley: Determine the level of road surface refinement, the larger, the finer
            this.minWidth = 100,
            this.maxWidth = 900,
            this.steerX = 0,
            this.horizon = 200,
        ...
        b.prototype.render = function (a) {
            ...
            for (var b = 500 / this.segNum, c = 0, d = 0; d < this.segNum; d++) this.tempInc = d,
            ...
            this.aEnemyCars[d].scale = ((this.aRowData[Math.floor(this.aEnemyCars[d].rowNum)].scale - 30) * this.roadScaleMultiplier + 30) / 730,
            ...
            this.aEnemyCars[d].y > 325 + 20 * this.hillAmount && this.aEnemyCars[d].x < this.canvasWidth / 2 + 80 && this.aEnemyCars[d].x > this.canvasWidth / 2 - 80 ? this.aEnemyCars[d].canHit && (this.aEnemyCars[d].canHit = !1, this.aEnemyCars[d].y > 380 + 20 * this.hillAmount && (this.aEnemyCars[d].speed = 0), this.callback("hitEnemyCar", {
    ...
    var b = function () {
        ...
        a.prototype.saveData = function () {
            if ("undefined" != typeof Storage) {
                for (var a = "", b = 0; b < this.aLevelStore.length; b++) a += this.aLevelStore[b],
...
false ? audioType = 0 : (audioType = 1, sound = new Howl({
    urls: ["audio/sound.mp3", "audio/sound.m4a"],
    sprite: {
        start1: [0, 700],
        start2: [1e3, 700],
        crash1: [2e3, 1200],
    ...
    urls: ["audio/music.mp3", "audio/music.m4a"],
    volume: .2,
    loop: !0
}));
var panel, hud, background, totalScore = 0,
    ...
    leftSteerSimple = 0,
    hillAmount, saveDataHandler = new Utils.SaveDataHandler("supersprintkarts", 9),
    ...
    raceLength, enemySpeed, racePos, carReleasedNum, carReleaseDelay, speedDifferencial, overtakenInc, bridgeDistanceTarg, firstPlay = !0,
    startTimer, aPowerUpBarData = new Array(0, 0, 0, 0),
    aPowerUpButsData = new Array(100, 100, 250, 250, 500, 500, 1e3, 1e3, 1e3),
    winnings = 0,
    accRate, nitroLength, endSoundPlayed, offRoad, startStage, musicTween, justSkid, logo, logoData;
