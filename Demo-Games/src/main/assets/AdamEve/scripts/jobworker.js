...
self.JobHandlers["ProcessImageData"] = function(a) {
    const b = a["buffer"]
      ...
      , d = a["width"]
      , e = a["height"];
    return a["flipY"] && FlipImageData(c, d, e),
...
self.addEventListener("message", (a)=>{
    const b = a.data
      , c = b["type"];
    return "init" === c ? (self.workerNumber = b["number"],
    self.dispatchPort = b["dispatch-port"],
    self.dispatchPort.onmessage = OnDispatchWorkerMessage,
    void (self.outputPort = b["output-port"])) : "terminate" === c ? void self.close() : void console.error("unknown message '" + c + "'")
}
);
function SendReady() {
...
function OnDispatchWorkerMessage(a) {
    console.log("OnDispatchWorkerMessage...1");
    const b = a.data
      , c = b["type"];
      console.log("OnDispatchWorkerMessage...2: " + c);
    if ("_import_scripts" === c)
        return void importScripts(...b["scripts"]);
    if ("_send_blob" === c)
        return void self.sentBlobs.set(b["id"], b["blob"]);
    if ("_send_buffer" === c)
    ...
    const d = b["jobId"]
      ...
      , e = b["params"];
    let g;
    if (self.activeJobId = d,
    !self.JobHandlers.hasOwnProperty(c))
