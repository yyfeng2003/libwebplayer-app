...
static FT_Stroker m_stroker = NULL;
/*
...
unsigned int draw_freetype_font(char** image, unsigned* image_width, unsigned* image_height, 
								void* font_info, unsigned long font_index, size_t font_size, unsigned char font_style,
								unsigned int x, unsigned int y, const char* cstr)
{
	...
	*/
	...
	if (FT_Set_Pixel_Sizes( face, font_size, 0 )) {
		return -1;
	}
 	...
 	if (*image)
	{
		...
		for ( ; *str != L'\0'; str++)
		{
			...
			if ((font_style & (unsigned char)kFreeTypeBold) > 0) {
				FT_Outline_Embolden( &glyph_slot->outline, 64 );
			}

			if ((font_style & (unsigned char)kFreeTypeItalic) > 0) {
				float lean = 0.25f;
				FT_Matrix matrix;
				matrix.xx = 0x10000L;
				...
				matrix.yx = 0;
				matrix.yy = 0x10000L;
				FT_Outline_Transform(&glyph_slot->outline, &matrix);
			}
...
unsigned int draw_stroke_font(	char** image, unsigned* image_width, unsigned* image_height, 
								void* font_info, unsigned long font_index, size_t font_size, unsigned char font_style,
								unsigned int x, unsigned int y, const char* cstr)
{
	if (!font_info) return -1;
 	...
 	if (*image)
	{
		...
		for ( ; *str != L'\0'; str++)
		{
			...
			FT_Stroker_Set(m_stroker, border_radius, FT_STROKER_LINECAP_ROUND, FT_STROKER_LINEJOIN_ROUND, 0 );
